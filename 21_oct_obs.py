...
def convertBallResult(ball):
    ...
    if ball[0] != None and ball[1] != None:
        ...
        res = None
        ballClose = True # because may be dribbling
...
def setdrive(dist, rps):
    if obsFlag == 'left':
        JUST_DRIVE_SYSTEM.SetTargetVelocities(0,-0.4)
        time.sleep(0.1)
        JUST_DRIVE_SYSTEM.SetTargetVelocities(-0.4,0)
        time.sleep(0.2)
    elif obsFlag == 'right':
        JUST_DRIVE_SYSTEM.SetTargetVelocities(0,0.4)
        time.sleep(0.1)
        JUST_DRIVE_SYSTEM.SetTargetVelocities(-0.4,0)
        time.sleep(0.2)
    elif BallInDribbler() == False and (lookingFor == 'yellow' or lookingFor == 'blue'):
        JUST_DRIVE_SYSTEM.SetTargetVelocities(boostScaling*max_fd_vel*(1-k*abs(rps/max_rvel)),boostScaling*rps)
    else:
