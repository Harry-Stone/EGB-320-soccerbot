...
obsMemFrames = 30 # number of frmaes untill an obstacle is forgotten
scoreGoal = 'yellow'
lookingFor = 'ball'
prevLook = 'ball'
max_fd_vel = 0.2
...
GPIO.setup(21, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) #goal switch bitch
...
def main():
    ...
    obstaclesRB = convertObsResult(vision.detectObstacles(hsv_frame,frame,True))
    vision.drawBall(ballRB,frame)

    if lookingFor == 'yellow':
        objectiveRB = convertGoalResult(vision.detectYellowGoal(hsv_frame,frame,cap))
    elif lookingFor == 'blue':
        objectiveRB = convertGoalResult(vision.detectBlueGoal(hsv_frame,frame,cap))
    else:
